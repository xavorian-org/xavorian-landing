  ...
  return (
    <section className="container mx-auto mt-20 sm:mt-28 lg:mt-32">
      <div className="grid gap-12 lg:grid-cols-[1fr_1.2fr] lg:gap-16 items-start">
        ...
        <ol ref={olRef} className="relative flex flex-col gap-5">
          <div
            className={cn('gate-line absolute bottom-6 left-[29px] top-6 w-px bg-border/80', drawn && 'gate-line--drawn')}
            aria-hidden="true"
          />
          {GATES.map((gate, i) => {
            const Icon = icons[i];
            return (
              <Reveal key={gate.id} delay={i * 110}>
                <li className="group relative flex gap-5 rounded-3xl border border-border bg-card p-6 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg sm:p-7">
          ...
          <Reveal delay={460} variant="scale">
            <li className="relative flex items-center gap-4 rounded-3xl border border-dashed border-border bg-muted/40 p-5 sm:p-6">
